...
#error Toradex hardening assumes CONFIG_UPDATE_TFTP is not set
     ...
     defined(CONFIG_BOOTMETH_CROS) ||          \
     defined(CONFIG_BOOTMETH_DISTRO) ||        \
     defined(CONFIG_BOOTMETH_EFILOADER) ||     \
     defined(CONFIG_BOOTMETH_EXTLINUX) ||      \
     defined(CONFIG_BOOTMETH_EXTLINUX_PXE) ||  \
     defined(CONFIG_BOOTMETH_GLOBAL) ||        \
     defined(CONFIG_BOOTMETH_SANDBOX) ||       \
     defined(CONFIG_BOOTMETH_VBE))
#error With the Toradex hardening, the only bootmeth that can be enabled is BOOTMETH_SCRIPT
#endif
